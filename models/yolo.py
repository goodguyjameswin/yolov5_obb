...
class Detect(nn.Module):  # 定义检测网络
    ...
    def __init__(self, nc=16, anchors=(), ch=()):  # detection layer
        ...
        self.register_buffer('anchor_grid',
                             a.clone().view(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,？(na),1,1,2) = (3, 1, 3, 1, 1, 2)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):  # nl = 3    in:(batch_size, no * na, size1, size2)
            ...
            if not self.training:  # inference推理模式
                ...
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[
                    i]  # xy 预测的真实坐标 y[..., 0:2] * 2. - 0.5是相对于左上角网格的偏移量； self.grid[i]是网格坐标索引
                # anchor_grid[i].shape=(1, 3, 1, 1, 2)  y[..., 2:4].shape=(bs, 3, height', width', 2)
                y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[
                    i]  # wh 预测的真实wh  self.anchor_grid[i]是原始anchors宽高  (y[..., 2:4] * 2) ** 2 是预测出的anchors的wh倍率
